...
void drawGraph(HWND hWnd, HDC hdc, PAINTSTRUCT ps, int isDirected) {
    int n = N;
    int **coords = getCoords(START);
    int centerX = findAverage(START, coords[0]);
    int centerY = findAverage(START, coords[1]); 
    int **matrix = isDirected ? generateAdir() : generateAundir();
...
void drawGraphTraversal(HWND hWnd, HDC hdc, PAINTSTRUCT ps, int* step, int isBFS) {
    int n = N;
    ...
    matrices searchMatrices = isBFS ? bfs(Adir) : dfs(Adir);
    int **treeMatrix = searchMatrices.treeMatrix;
    int **relativityMatrix = searchMatrices.relativityMatrix;

    int current = getVertex(*step, relativityMatrix);
    int from = getVertex(current, treeMatrix);

    HPEN OPen = CreatePen(PS_SOLID, 3, RGB(255, 140, 0));
    HPEN GPen = CreatePen(PS_SOLID, 3, RGB(0, 255, 0));
  
    SelectObject(hdc, GPen);
    if (*step != N && previous != -1) {
        int x1 = coords[0][from];
        int y1 = coords[1][from];
        int x2 = coords[0][current];
        int y2 = coords[1][current];

        if (Adir[from][current] == 1 && from != current) {
            if (Adir[current][from]) {
                drawCurvedEdge(hdc, x1, y1, x2, y2, 1);
            } else {
                drawEdge(hdc, current, from, x1, y1, x2, y2, centerX, centerY, 1);
            }
        }

        drawVertex(hdc, x1, y1, from);
    }

    if (previous != -1) {
        int x = coords[0][previous];
        int y = coords[1][previous];
        drawVertex(hdc, x, y, previous);

    }
    
    SelectObject(hdc, OPen);
    if (*step != N) {
        int x = coords[0][current];
        int y = coords[1][current];
        drawVertex(hdc, x, y, current);
    }
    ...
    freeCoords(coords);
    freeIntMatrix(Adir);
    freeIntMatrix(treeMatrix);
    freeIntMatrix(relativityMatrix);
};